...
class HomeFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        context?.registerReceiver(MyBroadcastReceiver(), IntentFilter(ConnectivityManager.CONNECTIVITY_ACTION))
    }
    ...
    override fun onResume() {
        super.onResume()
        MyBroadcastReceiver.connectivityReceiverListener = MyBroadcastReceiver.connectivityReceiverListener
    }

    private fun showNetworkMessage(isConnected: Boolean) {
        if (!isConnected) {
            snackBar = Snackbar.make(requireView().findViewById(R.id.homeView), "You are offline", Snackbar.LENGTH_LONG)
